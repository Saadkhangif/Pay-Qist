...
export default function CompleteProfilePage() {
  ...
  return (
    <section className="relative min-h-[calc(100dvh-4rem)] overflow-x-clip py-8 sm:py-12">
      ...
      <div className="relative mx-auto max-w-lg px-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:px-6">
        <div className="mb-4 flex items-center justify-between gap-3 px-0.5">
          ...
          <button
            type="button"
            onClick={logout}
            className="inline-flex min-h-11 items-center rounded-lg px-3 text-xs font-semibold text-slate-500 transition active:bg-slate-100 active:text-slate-700 dark:text-slate-400 dark:active:bg-surface-overlay dark:active:text-slate-200"
          >
            Sign out
          </button>
        </div>

        <div className="rounded-2xl border border-slate-200/80 bg-white/95 p-4 shadow-card backdrop-blur-sm dark:border-emerald-500/15 dark:bg-surface-raised/95 dark:shadow-dark-card sm:rounded-[2rem] sm:p-8">
          <CustomerProfileForm
